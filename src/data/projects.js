export const projects = [
  {
    title: "Agentic Evals for RAI",
    description:
      "Built an agentic workflow for evaluating RAI suggestions with a search and policy tool, and a router that helps pick relevant categories and guidelines from the policy doc to make better judgments. Improved eval quality numbers by 12% from baseline.",
  },
  {
    title: "Mimir",
    description:
      "A RAG-based bot for answering internal questions related to a team. Built using retrieval-augmented generation over internal documentation and deployed as a Microsoft Teams bot. Designed to be easily scalable to any team within Microsoft.",
  },
  {
    title: "Beyond Blocking",
    description:
      "Led a project at Microsoft to move from outright blocking of problematic search suggestions to generating safer replacements. Resulted in a ~5% reduction in the defect rate metric for Bing suggestions.",
  },
  {
    title: "Election Query Classifier",
    description:
      "Developed a contextual classifier model with 80/90 precision/recall for identifying election-related queries across major markets (US, EU). Deployed in the Copilot stack to enable appropriate content handling during elections.",
  },
];
